...
TODO create parametric env with boundary conditions
TODO sample from parametric env 

TODO set supporting joints before drawing 
TODO constraint on first frame

TODO create support + load environment
- create board with support and load conditions
    - 
- start with support instantiation

TODO create block_type mode 
TOOD 
...
class TrussFrameApp:
    ...
    '''
    def __init__(self, board_size_x, board_size_y, cell_size, square_size, figsize, verbose=True, block_type_mode=False):
        # settings for game 
        ...
        # series of graphs 
        self.graphs = [] # sequence of Graph objects that is created at each addition of frame
        self.vertices = dict() # dictionary of vertices with coordinate as key and Vertex object as value
        ...
        # intialize graph features  
            # set frames, graphs, vertices, supports, edges, maximal edges 

        ### FOR DEBUGGING ### 
        self.default_load = -0.4 # N applied per node
        # Define a dictionary where keys are points (x, y) and values are force vectors [Fx, Fy, Fz]
        # self.target_loads = {
        #         # (7, 5): [-4000.0, -8000.0, 0.0],  # Force at point (0, 0, 0)
        #         # (9, 5): [-4000.0, -8000.0, 0.0],  # Force at point (1, 0, 0)
        #         # (7, 5): [0.0, -8.0, 0.0],  # Force at point (0, 0, 0)
        #         (12, 6): [0.0, -10.0, 0.0],  # Force at point (1, 0, 0)
        #     }

        # Set Canitilever Env
        default_frames, supports, target_load = env_cantilever.set_cantilever_env(self.board_size_x, self.square_size)
        print(f'default_frames : {default_frames}, supports : {supports}, target load : {target_load}')
        # TODO update app values
    ...
    def update_drawing(self):
        ...
        for coord, vertex in self.vertices.items():
            self.ax.add_patch(patches.Circle(coord, radius=0.1, color='red'))
    ...
    def _draw_graph(self, graph_idx, ax, simplified=False):
        ...
        for coord, vertex in G.vertices.items():
            ax.add_patch(patches.Circle(coord, radius=0.1, color='black'))
            if not simplified:
    ...
    def view_truss_analysis(self, displacements):
        ...
        scrollable_box = widgets.Box([output_widget], layout=widgets.Layout(overflow_y='scroll', width = '200px', height='600px'))
...
def main():
    ...
    app = TrussFrameApp(board_size_x, board_size_y, cell_size, square_size, figsize, verbose)
    ...
    jl = juliacall.newmodule("TrussFrameRL") # TODO start julia session 
