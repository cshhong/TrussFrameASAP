...
FEA is ran at each step -> small reward at each step
'''
...
class CantileverEnv_1(gym.Env):
    ...
    def step(self, action):
        ...
        # FEA for every step : max deflection reward
        # if self.max_deflection > 0.15:
        #     reward -= self.max_deflection - 0.15
        # else:
        #     reward += 0.1
        ...
        # check failed element, give penalty 
        ...
        if end_bool==True:
            ...
            #     reward += 3 * self.allowable_deflection / self.max_deflection  # large reward for low deflection e.g. 0.5 / 0.01 = 50, scale for allowable displacement considering varying bc 
                reward += np.log(np.e + self.allowable_deflection / self.max_deflection)  # large reward for low deflection e.g. 0.5 / 0.01 = 50, scale for allowable displacement considering varying bc 
                # print(f"    Max Deflection : {self.max_deflection} Deflection Reward : {reward}")
    ...
    def apply_action(self, valid_action_tuple):
        ...
        # FEA at every step
        # self.update_displacement() # update displacement info in fea graph 
    ...
    def draw_fea_graph(self):
        ...
        for coord, load in self.curr_fea_graph.external_loads.items():
            ...
            if force_magnitude >= 0:
                arrow_dx = load[0] * 0.01 + 0.1
                arrow_dy = load[1] * 0.01 + 0.1
